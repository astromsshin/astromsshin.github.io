...
class curses_interface:
	...
	def run(self):
		...
		if use_fits > 0:
			use_ds9 = ds9.ds9(start=True,verify=True)
		while run_tag > 0:
			self.screen.clear()
...
if __name__ == '__main__':
	if len(sys.argv) < 3:
		print """PyVI.py [list filename] [log filename] -f -i [the directory name of image files] -t [the directory name of text files]
		Example) PyVI.py sel.list log.list -f -i ./Gallery -t ./Extra_text
		-f : if your image files are FITS files, use this option. PyVI will use the DS9 FITS viewer. You must open the DS9 with -xpa option.
	...
	dir_texts = "./"
	try:
		opts, args = getopt.getopt(sys.argv[3:], "fi:t:")
		for o, a in opts:
			if o == "-i":
				dir_images = a
			if o == "-t":
